...
public class EnchereServiceImpl implements EnchereService {
	...
	public Utilisateur creerCompte(Utilisateur utilisateur) {
		try {
			// Hachez le mot de passe avec bcrypt avant de le stocker dans la base de
			// données
			...
			// Gérer l'exception ici, par exemple, en journalisant l'erreur ou en effectuant
			// d'autres actions nécessaires.
			e.printStackTrace(); // Imprime la trace de la pile de l'exception
			return null; // Ou lancez une nouvelle exception, ou retournez une valeur par défaut, selon
	...
	public Utilisateur findUtilisateurByPseudoOuEmail(String identifiant) {
		try {
			...
			// d'autres actions nécessaires.
			e.printStackTrace(); // Imprime la trace de la pile de l'exception
			throw new UtilisateurNotFoundRuntimeException(); // Ou lancez une nouvelle exception, selon votre logique de
																// gestion d'erreur.
		}
	}
